...
export function App() {
  ...
  if (activeSector === 'PCP') return <div className="sector-portal pcp-portal"><header className="topbar"><strong>ER · PCP · {user.username}</strong><div className="user-area">{switcher}<button className="secondary" onClick={logout} disabled={loggingOut}>{loggingOut ? 'Saindo…' : 'Sair'}</button></div></header><main className="sector-workspace"><PcpPage /></main></div>;
