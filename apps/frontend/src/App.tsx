...
type Page = 'products' | 'batches' | 'stocks';
...
function BatchesPage({ canWrite }: { canWrite: boolean }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      const payload = { code: form.get('code'), expirationDate: form.get('expirationDate') || null };
  ...
  return <section className="card"><p className="eyebrow">Rastreabilidade</p><h2>Lotes</h2>{error && <p className="alert">{error}</p>}
    <div className="table-wrap"><table><thead><tr><th>Lote</th><th>Produto</th><th>Validade</th><th /></tr></thead><tbody>{batches.map((batch) => <tr key={batch.id}><td>{batch.code}</td><td>{productName(batch)}</td><td>{batch.expirationDate ?? 'Nao informada'}</td><td>{canWrite && <button className="secondary" onClick={() => setEditing(batch)}>Editar</button>}</td></tr>)}</tbody></table></div>
    {canWrite && <form key={editing?.id ?? 'new'} className="form-grid" onSubmit={(event) => void create(event)}><h3>{editing ? 'Editar lote' : 'Novo lote'}</h3><label>Produto<select name="productId" defaultValue={editing?.productId ?? ''} required disabled={Boolean(editing)}><option value="">Selecione</option>{products.map((product) => <option key={product.id} value={product.id}>{product.code} — {product.name}</option>)}</select></label><label>Codigo do lote<input name="code" defaultValue={editing?.code} required /></label><label>Validade<input name="expirationDate" type="date" defaultValue={editing?.expirationDate ?? ''} /></label><div className="actions"><button>Salvar lote</button>{editing && <button type="button" className="secondary" onClick={() => setEditing(null)}>Cancelar</button>}</div></form>}
  </section>;
}
...
function StocksPage({ canWrite }: { canWrite: boolean }) {
  ...
  return <section className="card"><p className="eyebrow">Estrutura logica</p><h2>Estoques e locais</h2><p className="muted">Cadastros de origem e destino. Saldos e movimentacoes nao fazem parte desta etapa.</p>{error && <p className="alert">{error}</p>}
...
export function App() {
  ...
  return <div className="app-shell">
    <header><div><p className="eyebrow">Estoque Revisao</p><strong>Cadastros base</strong></div><nav><button className={page === 'products' ? 'current' : ''} onClick={() => setPage('products')}>Produtos</button><button className={page === 'batches' ? 'current' : ''} onClick={() => setPage('batches')}>Lotes</button><button className={page === 'stocks' ? 'current' : ''} onClick={() => setPage('stocks')}>Estoques</button></nav><div className="user-area"><span>{user.username}</span><button className="secondary" onClick={() => void api.logout().finally(() => setUser(null))}>Sair</button></div></header>
    <main className="workspace">{page === 'products' && <ProductsPage canWrite={can('products.create') || can('products.update')} />}{page === 'batches' && <BatchesPage canWrite={can('batches.create')} />}{page === 'stocks' && <StocksPage canWrite={can('stocks.create') || can('stocks.update')} />}</main>
  </div>;
}
