...
function MovementsPage({ initialId, success, canCancel }: { initialId?: string; success?: string; canCancel: boolean }) {
  ...
  return <>
    ...
    <div className="content-grid"><section className="surface list-panel">{loading ? <LoadingState label="Carregando historico" /> : movements.length === 0 ? <EmptyState title="Nenhuma movimentacao encontrada" description="Ajuste os filtros ou registre uma operacao de estoque." /> : <div className="responsive-table"><table><thead><tr><th>Data</th><th>Tipo</th><th>Origem</th><th>Destino</th><th>Itens/quantidade</th></tr></thead><tbody>{movements.map((movement) => <tr key={movement.id} className={selected?.id === movement.id ? 'selected' : ''}><td data-label="Data"><button className="text-button" onClick={() => { setSelected(movement); revealDetails(); }}>{formatDateTime(movement.occurredAt)}</button></td><td data-label="Tipo">{label(movement.type)}</td><td data-label="Origem">{movement.originLocation.name}</td><td data-label="Destino">{movement.destinationLocation?.name ?? 'Varios destinos'}</td><td data-label="Itens/quantidade">{movement.items.length} / {movement.items.reduce((total, item) => total + item.quantity, 0)}</td></tr>)}</tbody></table></div>}</section>
      <aside className="surface detail-panel" tabIndex={-1}><p className="eyebrow">Detalhe imutavel</p><h2>{selected ? label(selected.type) : 'Selecione uma movimentacao'}</h2>{selected && <><dl><dt>Identificador</dt><dd>{selected.id}</dd><dt>Status</dt><dd><span className={`badge ${selected.status === 'EFETIVADA' ? 'active' : 'canceled'}`}>{selected.status === 'EFETIVADA' ? 'Efetivada' : 'Cancelada'}</span></dd><dt>Data/hora</dt><dd>{formatDateTime(selected.occurredAt)}</dd><dt>Responsavel</dt><dd>{selected.responsibleUser.username}</dd><dt>Origem</dt><dd>{selected.originLocation.name}</dd><dt>Destino</dt><dd>{selected.destinationLocation?.name ?? 'Distribuicao da revisao'}</dd><dt>Total de itens</dt><dd>{selected.items.length}</dd><dt>Quantidades de origem</dt><dd>{selected.items.map((item) => `${item.quantity} ${(item.productSnapshot ?? item.product).defaultUnit}`).join(' + ')}</dd><dt>Observacao</dt><dd>{selected.observation || '-'}</dd>{selected.status === 'CANCELADA' && <><dt>Cancelada em</dt><dd>{selected.canceledAt ? formatDateTime(selected.canceledAt) : '-'}</dd><dt>Cancelada por</dt><dd>{selected.canceledByUser?.username ?? '-'}</dd><dt>Motivo</dt><dd>{selected.cancellationReason}</dd></>}</dl><div className="divider" /><h3>Itens</h3><ul className="movement-detail-items">{selected.items.map((item) => <li key={item.id}><strong>{(item.productSnapshot ?? item.product).code} - {(item.productSnapshot ?? item.product).name}</strong>{selected.type === 'TRANSFERENCIA_INTERNA' ? <><span>Origem: lote {item.batch.code} / fabricação {formatDate(item.batch.manufacturingDate)} / validade {formatDate(item.batch.expirationDate)} / {selected.originLocation.name}</span><span>Destino: lote {item.destinationBatch?.code} / fabricação {formatDate(item.destinationBatch?.manufacturingDate ?? '')} / validade {formatDate(item.destinationBatch?.expirationDate ?? '')} / {selected.destinationLocation?.name}</span></> : <><span>Lote {item.batch.code}</span><span>Fabricacao {formatDate(item.batch.manufacturingDate)} - validade {formatDate(item.batch.expirationDate)}</span></>}<b>{item.quantity} {(item.productSnapshot ?? item.product).defaultUnit}</b>{item.outputProductSnapshot && <span>Desmontagem: {item.quantity} {(item.productSnapshot ?? item.product).defaultUnit} × {item.unitsPerPackage} → {item.outputQuantity} UN de {item.outputProductSnapshot.code} — {item.outputProductSnapshot.name}</span>}{item.distributions?.length > 0 && <ul className="distribution-detail">{item.distributions.map((distribution) => <li key={distribution.id}>{distribution.destinationLocation.name}: <strong>{distribution.quantity} {(item.outputProductSnapshot ?? item.productSnapshot ?? item.product).defaultUnit}</strong></li>)}</ul>}</li>)}</ul>{canCancel && !selected.shipmentId && selected.status === 'EFETIVADA' && <><div className="divider" /><button className="danger button-wide" onClick={() => setCancellationTarget(selected)}>Cancelar movimentacao</button></>}</>}</aside>
    </div>
...
export function App() {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">Ir para o conteúdo</a>
    ...
    <nav className="bottom-nav" aria-label="Navegação principal mobile">
      ...
      <NavButton active={['users', 'more', 'movements', 'reports', 'reports-reviews', 'reports-stock', 'products', 'stocks'].includes(page)} onClick={() => navigate('more')}>Menu</NavButton>
    </nav>
  </div>;
}
