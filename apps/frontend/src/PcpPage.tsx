...
export function PcpPage({ initialStatus = 'PENDENTE', initialId }: { initialStatus?: '' | 'PENDENTE' | 'EXECUTADA'; initialId?: string }) {
  ...
  return <>
    ...
    {selected && <Modal labelledBy="pcp-detail-title" className="pcp-detail-dialog" onClose={() => setSelected(null)}>
      ...
      {selected.shipmentEvidence.length > 0 && <><h3>Fotos/evidências</h3><div className="pcp-evidence-grid">{selected.shipmentEvidence.map((evidence) => { const item = selected.items.find((candidate) => candidate.productId === evidence.productId && candidate.batchId === evidence.batchId); return <ShipmentPhoto key={evidence.itemId} shipmentId={evidence.shipmentId} itemId={evidence.itemId} productName={(item?.productSnapshot ?? item?.product)?.name ?? 'item'} available={Boolean(evidence.photoMimeType)} additionalPhotos={evidence.additionalPhotos} />; })}</div></>}
